...
// `Section` renders a sentence-case label with no `num` (spec §1.3). This
// file proves the label lines up with its content (a) and is never a bare
// number (b), against real routes wherever the live dataset can exercise
// them (constraints.md: "every e2e assertion must hold for any valid
// dataset") and the `/preview/components` gallery otherwise (states the
// live data can't guarantee -- e.g. a person/publication detail page
...
test.describe('Section label layout', () => {
  ...
  // A `<p>` label and an `<h2>` label share the same `LABEL_CLASS` string,
  // but an unlayered base rule in styles/index.css (`p:not(:last-child) {
  // margin-bottom: 0.875rem }`) would otherwise only apply to the `<p>`
  // case -- `Section.tsx` sets `mb-0!` so both sit the same distance from
  // their content. Checked against the gallery's `gallery-home-a` instance,
  // which is guaranteed to render at least one of each ("Resources" is a
  // `<p>`; "Recent work" / "Outreach" / "The lab" are `<h2>`s) -- unlike a
  // live route, this doesn't depend on the dataset actually populating
  // every block.
...
test.describe('Section label never a bare number', () => {
  ...
  test('a real person profile and publication page never render one either', async ({ page }) => {
    ...
    // Checked immediately after the slug fetch, before any assertion -- an
    // empty dataset then reports a real skip, not a vacuous pass from two
    // `if` blocks that never ran.
...
// From `lg`, `PageTitle`'s `<h1>` and meta must sit in the same content
// column every `Section` below them uses -- `PageTitle` shares `Section`'s
// `SECTION_GRID`/`SECTION_GUTTER_X` (tokens.ts) with an empty label cell,
// rather than using the page's own left gutter directly, so the title
// doesn't jog sideways relative to the page's own body copy. Checked on
// every route that renders both a `PageTitle` and at least one `Section`
// below it.
